...
export async function GET(request: Request) {
  ...
  const stored = await d1.prepare("SELECT id, tenant_id, redirect_uri FROM oauth_states WHERE provider = 'google_calendar' AND state_hash = ? AND consumed_at IS NULL AND expires_at > CURRENT_TIMESTAMP LIMIT 1")
    .bind(stateHash).first<{ id: string; tenant_id: string; redirect_uri: string }>();
  ...
  const membership = await d1.prepare("SELECT id FROM tenant_members WHERE tenant_id = ? AND email = ? AND is_active = 1 LIMIT 1")
    .bind(stored.tenant_id, user.email.toLowerCase()).first<{ id: string }>();
  if (!membership) return jsonError("Seu usuário não pode alterar esta integração.", 403, "FORBIDDEN");
  ...
  await d1.batch([
    d1.prepare("INSERT INTO integration_connections (id, tenant_id, provider, status, encrypted_credentials, configuration_json) VALUES (?, ?, 'google_calendar', 'connected', ?, ?) ON CONFLICT (tenant_id, provider) DO UPDATE SET status = 'connected', encrypted_credentials = excluded.encrypted_credentials, configuration_json = excluded.configuration_json, updated_at = CURRENT_TIMESTAMP")
      .bind(crypto.randomUUID(), stored.tenant_id, encrypted, JSON.stringify({ calendarId: "primary", scope: token.scope })),
    d1.prepare("UPDATE oauth_states SET consumed_at = CURRENT_TIMESTAMP WHERE id = ?").bind(stored.id),
  ]);
  return Response.redirect(`${url.origin}/admin?integration=google-connected`);
}
